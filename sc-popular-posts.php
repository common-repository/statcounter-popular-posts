...
?>
  ...
  <form action="" method="POST">
    <input type="text" name="SPPid" value="<?php echo $SPPid;?>" size="40" placeholder="The ID that starts with p" /><br/>
...
?>
  ...
  <select class="widefat" id="<?php echo $this->get_field_id('type'); ?>" name="<?php echo $this->get_field_name( 'type' ); ?>">
    ...
    <option value="visits" <?php echo $instance['type']=="visits" ? "selected":""?>>Visits</option>
  </select>
  <p>The number of items to be displayed</p>
  <input type="number" id="<?php echo $this->get_field_id('items'); ?>" name="<?php echo $this->get_field_name( 'items' ); ?>" value="<?php echo isset($instance['items']) ? $instance['items']:10;?>" />
  <br/><br/>
<?php  
  }
  ...
  public function makeList($url){
    ...
    foreach($html->find("table.results td") as $tdElem){
      ...
      if(isset($aElem->href)){
        ...
        if(!isset($duplicates[$current_url])){
